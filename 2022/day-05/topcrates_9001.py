...
with open('input.txt') as f:
    ...
    for line in moves:
        ...
        from_n = len(stacks[move_from-1])
        if from_n < move_n:
            print(f"error, not enough crates in stack {move_from} for move: {line}")
        crates = stacks[move_from-1][from_n - move_n:]
        stacks[move_from-1] = stacks[move_from-1][0: from_n - move_n]
        stacks[move_to-1] = stacks[move_to-1] + crates
